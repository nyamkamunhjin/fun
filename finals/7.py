...
class G:
    ...
    def dfs(self, s, edges, visited=[]):
        visited = visited + [s]

        if len(self.V) == len(visited):
            # print(visited)
            return visited
        
        # neighbors of current node 
        neighbors = [x for x in edges if x[0] == s]

        for i in neighbors:
            if i[1] not in visited:
                visited = self.dfs(i[1], edges, visited)
    ...
    def find_bridges(self):
        ...
        for i in E:
            ...
            if len(self.dfs(self.V[0], temp_E)) != len(self.V):
                bridges.append(i)
...
g1 = G([1, 2, 3, 4, 5], [(1, 2, 10), (1, 4, 7), (2, 3, 18), (4, 5, 2), (3, 5, 1), (2, 4, 9), (1, 3, 9)])
g1.find_bridges()
